function calcularepFcn() {
    ...
    Vtapas = Math.PI/12*(Math.pow(diametro + 2*espesortapareal,3) - Math.pow(diametro,3));
        ...
        /*Peso de equipo lleno*/
        R = diametro / 2;
        theta = Math.acos((R - nivel) / R);
        thetaGrados = (theta * 180) / Math.PI;  // conversión a grados
        Vliquido = Math.pow(R, 2) * diametro * ((thetaGrados) - Math.sin(theta) * Math.cos(theta));  // Añadido el operador de multiplicación antes de diametro
        Pliquido = (Vliquido * rho);
        pesoequipolleno = (Pliquido + pesoequipo);
